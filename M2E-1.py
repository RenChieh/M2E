...
class Event:
    ...
    def event1_player_log_in(self,player_id):
        ...
        # 如果該玩家尚未在 player_data 中建立資料，則初始化該玩家的資料字典
        current_age = 30
        currnet_dayweek = 7
        current_runtype = "E"
        current_kmday = 42
        ...
        current_mileage = 100
        current_wear_rate_tolerate = 1
        hurt_rate = 0

        self.player_data[player_id] = {
              "player": player_id,
              "age": current_age,
              "days/week": currnet_dayweek,
              "run_type": current_runtype,
              "km/day":current_kmday,
              "shoes_type": current_shoes_type,
              "place": current_place,
              "mileage": current_mileage,
              "wear_rate_tolerate": round(current_wear_rate_tolerate,2),
              "hurt_rate": round(hurt_rate,2)}
        # print(self.player_data[player_id])
...
def main(discount):
  ...
  for player in range(1, player_amount + 1):
      ...
      player_profile = event.event1_player_log_in(player_id)
      # 將玩家資料存儲到 player_profiles 中
      player_profiles["player_id"].append(player_profile["player"])
      player_profiles["age"].append(player_profile["age"])
      player_profiles["days/week"].append(player_profile["days/week"])
      player_profiles["run_type"].append(player_profile["run_type"])
      player_profiles["km/day"].append(player_profile["km/day"])
      player_profiles["shoes_type"].append(player_profile["shoes_type"])
      player_profiles["place"].append(player_profile["place"])
      player_profiles["mileage"].append(player_profile["mileage"])
      player_profiles["wear_rate_tolerate"].append(player_profile["wear_rate_tolerate"])
      player_profiles["hurt_rate"].append(player_profile["hurt_rate"])
...
plt.plot(results_df['weight'], results_df['Profit_Margin'], label='Profit Margin', marker='o')

plt.grid(True)
plt.xlabel('weight')
plt.ylabel('Profit_Margin')
plt.title('weight vs Profit Margin')
plt.legend()
plt.show()